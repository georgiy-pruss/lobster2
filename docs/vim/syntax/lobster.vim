...
syn keyword    lobsterStatement    return import namespace pakfile  assert
...
" control
syn keyword    lobsterConditional    if else elif switch
...
" Integers
syn match      lobsterDecimalInt        "\<\d[0-9_]*\%([Ee][-+]\=\d\+\)\=\>"
syn match      lobsterHexadecimalInt    "\<0[xX][0-9A-Fa-f_]\+\>"
syn match      lobsterHexadecimalInt    "#[0-9A-Fa-f_]\+\>"
hi def link    lobsterDecimalInt        Integer
hi def link    lobsterHexadecimalInt    Integer
hi def link    Integer                  Number

" Floating point
syn match      lobsterFloat    "\<\d[0-9_]*\.[0-9_]*\%([Ee][-+]\=\d\+\)\=\>"
syn match      lobsterFloat    "\<\.\d[0-9_]*\%([Ee][-+]\=\d\+\)\=\>"
syn match      lobsterFloat    "\<\d[0-9_]*\%([Ee][-+]\=\d\+\)\=\>"
